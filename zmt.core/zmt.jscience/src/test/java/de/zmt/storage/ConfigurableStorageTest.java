...
public class ConfigurableStorageTest {
	...
	assertEquals("Storage did not store correct amount: ", STORED_OUT, storedOut);

	assertEquals("Amount retrieved when clearing storage was not correct: ", RETRIEVED, storage.clear());

	// add excess amount
	logger.info("adding " + EXCESS_CHANGE_AMOUNT);
	Amount<Dimensionless> rejectedIn = storage.add(EXCESS_CHANGE_AMOUNT).getRejected();
	assertEquals("Storage did not reject correct amount: ", REJECTED_IN, rejectedIn);
	...
	assertEquals("Storage did not reject correct amount: ", REJECTED_OUT, rejectedOut);
     ...
     * error value.
     */
    private static final Amount<Dimensionless> RESULT = AmountUtil.zero(CHANGE_AMOUNT).plus(CHANGE_AMOUNT.times(1d))
	    .plus(CHANGE_AMOUNT.times(1d)).plus(CHANGE_AMOUNT.opposite().times(1d));
