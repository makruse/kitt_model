...
/**
 * Fish
 * implements the behaviour of the fish<br />
 * 
 * @author oth
 */
public class Fish extends Agent{
	...
	/** the memory of fish's locations. for each cell a counter is increased when the fish has shown up there*/
	 ...
	 */
	public Fish(final double x, final double y, final double z, double initialBiomass, double initialSize, Environment environment,ModelParams params, SpeciesDefinition speciesDefinition)
	{	
...
// 				}
	...
	/**
	 * searches a randomly drawn position in suitable habitat for resting and feeding
	 */
	...
	protected void move() {
		...
		double scal=100.0; // get the distance scaled to a value between 0 and 10	for input in tanh function	
		double dist=0.0;						
		...
		if( dielCycle == DielCycle.SUNRISE || (environment.getHabitatOnPosition(pos) == HabitatHerbivore.SEAGRASS.id )){
			dist=getDistance(pos,centerOfAttrForaging);
			attractionDir=getDirectionToAttraction(pos,centerOfAttrForaging);
		}
		else if( dielCycle == DielCycle.SUNSET || (environment.getHabitatOnPosition(pos) == HabitatHerbivore.CORALREEF.id )){
		...
		// step = cm per time step (stimmt das mit cm??) => abhängig von size(bodylength), cell resolution und time resolution
		//step=6; //size/params.environmentDefinition.cellResolution * speciesDefinition.stepMigration * 60 * params.environmentDefinition.timeResolutionMinutes;		
		
		// migrating -biased correlated random walk when sunrise and far from attraction
		if( p > 0.1){
			step=6;
			double newXSpeed=0;
			double newYSpeed=0;
			...
			newXSpeed = xSpeed + (p * attractionDir.x * step ) +  (1-p) * environment.rand.nextGaussian() * step ;		
			newYSpeed = ySpeed + (p * attractionDir.y * step ) +  (1-p) * environment.rand.nextGaussian() * step ;
			
			int targetHabitat = environment.getHabitatOnPosition(new Double3D( pos.x+newXSpeed, pos.y + newYSpeed,0.0));
			if(targetHabitat==HabitatHerbivore.MAINLAND.id){
				newXSpeed = (p * attractionDir.x * environment.rand.nextGaussian() * step ) - newXSpeed * speedup;		
				newYSpeed = (p * attractionDir.y * environment.rand.nextGaussian() * step ) - newYSpeed * speedup;
			}						
			xSpeed=newXSpeed;
			ySpeed=newYSpeed;			
		}		
			...
			if( dielCycle == DielCycle.DAY){
				...
				int counter=0;
				do{
					newXSpeed = xSpeed * 0.2 + environment.rand.nextGaussian() * step;		
					newYSpeed = ySpeed * 0.2 + environment.rand.nextGaussian() * step;
					
					Double3D posCandidate=new Double3D( pos.x + xSpeed, pos.y + ySpeed,0);
			...
			else if( dielCycle == DielCycle.NIGHT || dielCycle==DielCycle.LATE_NIGHT){
				step=1;
				int counter=0;
				do{
					newXSpeed = xSpeed * 0.1 + environment.rand.nextGaussian() * step;		
					newYSpeed = ySpeed * 0.1 + environment.rand.nextGaussian() * step;
		...
		else if(xSpeed < -speedMax) xSpeed=-speedMax;
		if (ySpeed > speedMax) ySpeed=speedMax;		
		else if(ySpeed < -speedMax) ySpeed=-speedMax;
						
		double newX = pos.x + xSpeed;
		double newY = pos.y + ySpeed;
		
		if( (newX > environment.getFieldSizeX() || newX < 0) || 
				environment.getHabitatOnPosition(new Double3D(newX,newY,0.0))==HabitatHerbivore.MAINLAND.id){			
			xSpeed=-xSpeed;
			newX = pos.x + xSpeed;
			// if vector intersects border then reflect at border 
//			Double2D reflected = Vec.reflectVector(new Double2D(xSpeed,ySpeed), new Double2D(0,1));	
		}
		...
		pos=new Double3D(newX ,newY, 0);
		Double2D newPos= new Double2D(newX,newY);
		
		environment.field.setObjectLocation(this, newPos);		
