...
import sim.engine.*;
import sim.app.hexabugs.HexaBugsWithUI.HexagonChoice;
import sim.display.*;
...
import de.zmt.kitt.gui.CustomConsole;
...
public class Gui extends GUIState implements ActionListener,TableModelListener
{
	...
	public static String getName() { return "Kitty"; }
	
	/** called when start was pressed on the control panel. 
	 * reinitializes the displays and calls the sim.start() method*/
	public void start()
	{
		super.start();
		setupPortrayals();
	...
	{	
		...
		//kittyView = new FishView(Color.green,6.0, sim.cfg.speciesList.size());
		...
		foodGridPortrayal.setMap(
	    new sim.util.gui.SimpleColorMap(
	                0.0,14.0,new Color(0,0,0),new Color(0,250,0)));
		...
		memoryGridView = new  MemoryCellsView( this, sim.params, display.getWidth(), display.getHeight());
	...
	public void init(Controller c)
	{
		...
		display = new Display2D( w+1,h+1,this);  // y contains the field height plus timeview height
	    ...
	    EnvironmentDefinitionView environmentDefinitionView = new EnvironmentDefinitionView( sim.params.environmentDefinition,  this);
		...
		for( SpeciesDefinition def: sim.params.speciesList){
		    ...
		    SpeciesDefinitionView speciesDefinitionView = new SpeciesDefinitionView( idx, def,  this);
		    console.getTabPane().addTab("Species:" + def.speciesName,speciesDefinitionView);	
		}
	    	    	    
		displayFrame.setVisible(true);
		
		JMenuItem menuItemOpen = (JMenuItem) console.getJMenuBar().getMenu(0).getMenuComponent(1);
		menuItemOpen.setText("open configuration..");
	    JMenuItem menuItemSaveAs= (JMenuItem) console.getJMenuBar().getMenu(0).getMenuComponent(3);
	    menuItemSaveAs.setText("save configuration as..");
	    
		JMenuItem menuItemSave= (JMenuItem)console.getJMenuBar().getMenu(0).getMenuComponent(2);
		JMenuItem menuItemNew= (JMenuItem)console.getJMenuBar().getMenu(0).getMenuComponent(0);
	...
	public void reload(String path){
		...
		sim.reload(path);
		//! actualize gui elements with new values read from config file
		// setupGui();
	}
    ...
    public Inspector getInspector()
    {
	    ...
	    final SimpleInspector hexInspector = new SimpleInspector(new Object(),this);
	...
	{		
		Console c = new CustomConsole(new Gui("resource/ModelParams.xml"));
		c.setVisible(true);	
	}
